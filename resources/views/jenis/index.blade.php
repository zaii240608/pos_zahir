...
@section('content')
    ...
    <div class="min-h-screen bg-stone-100/70 py-8">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-6">
            ...
            <!-- Header Halaman -->
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 bg-white p-6 rounded-3xl border border-stone-200/80 shadow-xs">
                <div>
                    <h1 class="text-2xl font-extrabold text-stone-900 tracking-tight">Kategori / Jenis Produk</h1>
                    <p class="text-xs text-stone-500 mt-1">Kelola jenis dan kelompok kategori untuk barang/produk kamu.</p>
                </div>

                <a href="{{ route('admin.jenis.create') }}" 
                   class="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-teal-600 hover:bg-teal-700 text-white rounded-2xl text-xs font-bold transition-all shadow-xs hover:shadow-md">
                    ...
                    <table class="w-full text-left border-collapse">
                        <thead>
                            <tr class="bg-stone-50/80 border-b border-stone-100 text-[11px] font-bold text-stone-400 uppercase tracking-wider">
                                ...
                                <th class="py-4 px-6">Nama Jenis</th>
                        ...
                        <tbody class="divide-y divide-stone-100 text-xs text-stone-700 font-medium">
                            @forelse($jenisList as $index => $item)
                                <tr class="hover:bg-stone-50/50 transition-colors">
                                    <td class="py-4 px-6 text-stone-400 font-bold">
                                        {{ $jenisList->firstItem() + $index }}
                                    </td>
                                    <td class="py-4 px-6">
                                        <span class="font-extrabold text-stone-900">{{ $item->nama_jenis }}</span>
                                    </td>
                                    <td class="py-4 px-6">
