            ...
            <div class="bg-white rounded-xl border border-slate-200/80 shadow-sm p-5 space-y-6">
                ...
                <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-3">
                    ...
                    <div class="flex items-center gap-2 text-xs font-medium text-slate-500">
                        ...
                        <span class="px-2.5 py-1 rounded-md bg-slate-100 text-slate-700 font-semibold">Semua Minggu</span>
                    </div>
                </div>
                ...
                @if(!empty($historyMingguan) && count($historyMingguan) > 0) 
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                        ...
                        <div>
                            <p class="text-[10px] font-bold text-slate-400 uppercase tracking-wider">RATA-RATA / MINGGU</p>
                            <h2 class="text-xl font-bold text-slate-800 mt-0.5">Rp {{ $formatRingkas($rataRataMingguan) }}</h2>
                        </div>
                        <div>
                            <p class="text-[10px] font-bold text-rose-500 uppercase tracking-wider flex items-center gap-1.5">
                                <span class="w-1.5 h-1.5 rounded-full bg-rose-500 inline-block"></span> MINGGU AKTIF
                            </p>
                    ...
                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-3 pt-2">
                        ...
                        <div class="bg-rose-50/60 border border-rose-200/80 p-3.5 rounded-lg">
                            <p class="text-[11px] font-medium text-rose-800">Total Minggu Tercatat</p>
                            <h3 class="text-base font-bold text-slate-800 mt-1">
                                {{ $totalMingguTercatat }} <span class="text-[11px] font-normal text-rose-600">Minggu</span>
                            </h3>
                        </div>
                    </div>
                    ...
                    <div class="pt-4 border-t border-slate-100 space-y-3">
                        ...
                        @foreach($historyMingguan as $index => $item) 
                            ...
                            <div x-data="{ open: {{ $index === 0 ? 'true' : 'false' }} }" 
                                 class="border border-slate-200/80 rounded-xl overflow-hidden bg-white transition-all duration-200">
                                
                                <div @click="open = !open" 
                                     class="p-4 bg-white hover:bg-slate-50/80 cursor-pointer flex items-center justify-between transition-colors">
                                    ...
                                    <div class="flex items-center gap-3">
                                        ...
                                        <div>
                                            <div class="flex items-center gap-2">
                                                ...
                                                @if($index === 0)
                                                    <span class="px-1.5 py-0.5 text-[9px] font-bold bg-rose-100 text-rose-600 rounded uppercase">Terbaru</span>
                                                @endif 
                                    ...
                                    <div class="text-right flex items-center gap-4">
                                        @if($mingguLalu) 
                                            @if($diffPercent > 0)
                                                <span class="hidden sm:inline-flex items-center gap-0.5 text-[11px] font-bold text-emerald-600 bg-emerald-50 px-2 py-0.5 rounded">
                                                    ▲ +{{ number_format($diffPercent, 1) }}%
                                                </span>
                                            @elseif($diffPercent < 0)
                                                <span class="hidden sm:inline-flex items-center gap-0.5 text-[11px] font-bold text-rose-600 bg-rose-50 px-2 py-0.5 rounded">
                                                    ▼ {{ number_format($diffPercent, 1) }}%
                                                </span>
                                            @endif 
                                        @endif
                                ...
                                <div x-show="open" x-collapse x-cloak class="border-t border-slate-100 bg-slate-50/50 p-4">
                                    @if(count($detailHarian) > 0)
                                        <div class="overflow-x-auto rounded-lg border border-slate-200/60 bg-white">
                                            <table class="w-full text-left text-xs border-collapse">
                                                <thead>
                                                    <tr class="bg-slate-50/80 border-b border-slate-200/60 text-slate-500 font-bold uppercase tracking-wider">
                                                            ...
                                                            <td class="py-2.5 px-4 text-center">
                                                                @if($tglRaw)
                                                                    <a href="{{ route('admin.history.date', ['tanggal' => $tglRaw]) }}" 
                                                                       class="inline-flex items-center gap-1 text-[10px] font-semibold text-rose-600 hover:text-rose-800 bg-rose-50 hover:bg-rose-100 px-2 py-1 rounded transition-colors">
